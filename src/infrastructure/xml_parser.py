...
class XMLParser:
    ...
    def parse(self):
        ...
        for i in self.xml_head_objects:
            ...
            if element_tag == XMLHeadObjects.ANALOGS.value:
                ...
                for iter_obj in element_obj:
                    signal = self._parse_discrete(iter_obj)
                    output[element_tag].append(signal)
    ...
    def _parse_analog(self, iter_obj) -> AnalogSignal:
        ...
        try:
            ...
            # TODO driver.find('Type').text — пригодится при парсинге не только MB
            ...
            raise ValueError(f'Ошибка парсинга аналогового сигнала "{tag}": {e}') from e

    def _parse_discrete(self, iter_obj) -> DiscreteSignal:
        tag = iter_obj.find('Name').text
        try:
            direction = Direction(iter_obj.find('Direction').text)
            name = iter_obj.find('Description').text
            ...
            inversed = iter_obj.find('Inversed').text.lower() == 'true'
